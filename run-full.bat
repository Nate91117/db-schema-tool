@echo off
REM Full pipeline: Stage 1 + Stage 2 (Haiku) + Stage 3 (Sonnet, top 75 tables only).
...
echo Running full pipeline (caps Stage 3 at 75 tables for cost control)...
...
echo.
dbscan --max-stage3-tables 75 --skip-stage1 --output results.json
